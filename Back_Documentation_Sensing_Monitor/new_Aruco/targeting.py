...
# can be imported when this script is executed directly.
import os
import sys
...
def get_pose_points(pose_result, width, height):
    """Returns pixel-space hip points from a PoseLandmarker result, or None.

    Only the hips are used -- every muscle group we target is a lower-back
    / lumbar-region landmark, so shoulder tracking was dropped to reduce
    noise (shoulders move independently of the lower back and don't help
...
def compute_targets(pose_points, mirror=False):
    """
    Build a hip-only frame and place every muscle group per
    targeting_config.MUSCLE_GROUP_RATIOS.

    The hip line gives the lateral (left-right) direction directly. The
    vertical ("up the back") direction is the perpendicular to the hip
    line, picked to point toward the head. Each muscle group sits some
    fraction of a hip-width above that line, per t_above_hip.
    ...
    lateral_ref = _sub(hip_r, hip_l)
    lateral_unit = _unit(lateral_ref)
    if mirror:
        lateral_unit = _scale(lateral_unit, -1)

    # Perpendicular to the hip line, picked to point "up" (negative y in
    # image coordinates, since y grows downward).
    perp_a = (-lateral_ref[1], lateral_ref[0])
    perp_b = (lateral_ref[1], -lateral_ref[0])
    vertical_ref = perp_a if perp_a[1] < 0 else perp_b
    vertical_unit = _unit(vertical_ref)

    targets = {}
    for muscle, (t_above_hip, lateral_frac) in cfg.MUSCLE_GROUP_RATIOS.items():
        base = _add(hip_mid, _scale(vertical_unit, t_above_hip * hip_width))
        for side, sign in (("R", 1.0), ("L", -1.0)):
            offset = _scale(lateral_unit, sign * lateral_frac * hip_width)
...
def draw_o(frame, center, radius=cfg.CAPTURE_RADIUS_PX, color=cfg.COLOR_TARGET, thickness=3):
    cv2.circle(frame, (int(center[0]), int(center[1])), int(radius), color, thickness, cv2.LINE_AA)
